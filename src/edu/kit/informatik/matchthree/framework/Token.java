...
public final class Token implements Comparable<Token> {
     ...
     * Empty tokens (whitespace) are ignored.
     * 
     * @param tokenString
     ...
     * tokens.
     * <p>
     * Spaces in {@code tokenString} are ignored by the iterator.
     * 
     * @param tokenString
     ...
     * @see #stream(String)
     */
    public static Stream<Token> streamWithWhitespaceAsNull(String tokenString) {
        Objects.requireNonNull(tokenString, "Token string to stream must not be null");
        return tokenString.chars().mapToObj(it -> new String(new int[] {it}, 0, 1))
                .map(it -> it.trim().isEmpty() ? null : new Token(it));
    }
     ...
     * @return an iterator of all tokens that are included in
     *         {@code tokenString}.
     * @see #streamWithWhitespaceAsNull(String)
     */
    public static Iterator<Token> iteratorWithWhitespaceAsNull(String tokenString) {
        Objects.requireNonNull(tokenString, "Token string to convert to iterator must not be null");
        return streamWithWhitespaceAsNull(tokenString).iterator();
    }
}
